...
(defun parse-basetype (node vk-spec)
  ...
  (let* ((attributes (attrib-names node))
    ...
    (let ((base-type-name (if (> (length (type-name type-info)) 0)
                              (type-name type-info)
                              "void")))
...
(defun parse-funcpointer (node vk-spec)
  ...
  (let ((requirements (xps (xpath:evaluate "@requires" node)))
        (name (or (xps (xpath:evaluate "name" node))
                  (xps (xpath:evaluate "proto/name" node)))))
...
See VULKAN-SPEC
"
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[(@category=\"include\")][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-type-include node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[not(@category)][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-requires node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"basetype\"][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-basetype node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"bitmask\"][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-bitmask node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"define\"][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-define node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"enum\"][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-enum-type node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"handle\"][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-handle node vk-spec))
  ;; Newer registries may place an alias before its canonical structure.
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"struct\" or @category=\"union\"][not(@alias)][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-struct node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"struct\" or @category=\"union\"][@alias][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))
    (parse-struct node vk-spec))
  (xpath:do-node-set (node (xpath:evaluate "/registry/types/type[@category=\"funcpointer\"][not(@api) or contains(concat(',', @api, ','), ',vulkan,')]" vk.xml))

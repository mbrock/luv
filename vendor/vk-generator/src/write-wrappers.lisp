...
(defun get-type-to-declare (type-name vk-spec &optional param vector-params)
  (cond
    ...
    ((or (let ((enum (or (gethash type-name (enums vk-spec))
                         (find-if (lambda (candidate)
                                    (member type-name (alias candidate) :test #'string=))
                                  (alexandria:hash-table-values (enums vk-spec))))))
           (and enum (not (is-bitmask-p enum))))
...
(defun write-typed-command (out command vk-spec)
...
(defun write-command (out command vk-spec)
  (handler-case
      (write-typed-command out command vk-spec)
    (vulkan-spec::command-classification-error (condition)
      (warn "Generating a raw passthrough for ~a: ~a" (name command) condition)
      (let ((fixed-function-name (fix-function-name (name command) (tags vk-spec))))
        (format out "~%~((defun ~a (&rest args)~)" fixed-function-name)
        (format out "~%  ~s" "Raw pointer-level fallback for a command the high-level wrapper generator cannot yet classify.")
        (format out "~%  (apply #'%vk:~(~a~) args))~%" fixed-function-name)))))
...
(defun write-vk-functions (vk-functions-file vk-spec &optional dry-run)
  (flet ((write-commands (stream)
           ...
           ;; Some unclassified extension enumerate wrappers currently make
           ;; SBCL infer a Lisp value where generated CFFI code asserts a
           ;; pointer.  Their diagnostics should not make the otherwise
           ;; usable generated FASL fail ASDF's compilation step.
           (format stream "#+sbcl~%(declaim (sb-ext:muffle-conditions sb-int:type-warning))~%~%")

...
(define-test ask-returns-before-provider-turn-completes
    ...
    (unwind-protect
         (progn
           ...
           (let* ((before (get-internal-real-time))
             ...
             (true (< elapsed 0.1) "ASK only creates a worker")
             (true (sb-thread:wait-on-semaphore started :timeout 1.0))
             (true (member (agent:turn-status turn)
                           '(:thinking :working)))
             (sb-thread:signal-semaphore finish)
             (true (not (eq :timeout (join-test-turn turn))))
             (true (eq :done (agent:turn-status turn)))
             (true (string= "unblocked" (agent:turn-text turn)))))
...
(define-test asks-reach-the-provider-in-publication-order
  (let ((started (sb-thread:make-semaphore :count 0))
        (finish (sb-thread:make-semaphore :count 0))
        (received '())
         ...
         (progn
           ...
           (let ((first (agent:ask "first" :agent application-agent)))
             ...
             (let ((second (agent:ask "second" :agent application-agent))
               ...
               (true (eq (agent:turn-thread first) (agent:turn-thread second)))
               (true (eq (agent:turn-thread second) (agent:turn-thread third)))
               (sb-thread:signal-semaphore finish)
               (true (agent:wait-for-turn first :timeout 1.0))
               (true (agent:wait-for-turn second :timeout 1.0))
               (true (agent:wait-for-turn third :timeout 1.0))
               (true (equal '("first" "second" "third") received)))))
      (sb-thread:signal-semaphore finish)
      (when application-agent
        (agent:release-application-agent application-agent)
        (agent:wait-for-application-agent-release
...
(define-test release-prompts-close-and-unblocks-an-active-turn
  (let ((started (sb-thread:make-semaphore :count 0))
             ...
             (let* ((queued (agent:ask "queued" :agent application-agent))
                    ...
                    (elapsed
                      (/ (- (get-internal-real-time) before)
                         (float internal-time-units-per-second 1.0))))
               (true released)
               (true (< elapsed 0.1) "release only publishes terminal work")
               (true (eq :released
                         (agent:application-agent-state application-agent)))
               (true (null (agent:application-agent-application
                            application-agent)))
               (true (agent:wait-for-application-agent-release
                      application-agent :timeout 1.0))
               (true (agent:wait-for-turn active :timeout 1.0))
               (true (agent:wait-for-turn queued :timeout 1.0))
               (true (eq :done (agent:turn-status active)))
               (true (eq :failed (agent:turn-status queued)))
               (true (typep (agent:turn-error queued)
                            'agent:application-agent-released))
               (true (equal '(:close :turn) (nreverse events)))
               (true (= 1 close-count))
               (true (not (eq (agent:application-agent-close-thread
                               application-agent)
                              (agent:application-agent-worker-thread
                               application-agent))))
               (true (agent:application-agent-release-finished-p
                      application-agent))
               (true (null (agent:release-application-agent
                            application-agent))))))
      (sb-thread:signal-semaphore provider-closed)
      (when application-agent
        (agent:release-application-agent application-agent)
...
(define-test stalled-provider-close-has-a-bounded-observable-wait
  (let ((close-started (sb-thread:make-semaphore :count 0))
    ...
    (unwind-protect
         (progn
           ...
           (true (agent:release-application-agent application-agent))
           (true (sb-thread:wait-on-semaphore close-started :timeout 1.0))
           (true (eq :released
                     (agent:application-agent-state application-agent)))
           (true (null (agent:application-agent-application
                        application-agent)))
           (false (agent:wait-for-application-agent-release
                   application-agent :timeout 0.05))
           (false (agent:application-agent-close-finished-p application-agent))
           (false (agent:application-agent-release-finished-p application-agent))
           (sb-thread:signal-semaphore finish-close)
           (true (agent:wait-for-application-agent-release
                  application-agent :timeout 1.0))
           (true (agent:application-agent-close-finished-p application-agent))
           (true (agent:application-agent-release-finished-p
                  application-agent)))
      (sb-thread:signal-semaphore finish-close)
      (when application-agent
...
(define-test command-schema-and-parser-share-clim-argument-semantics
...
(define-test command-effects-cross-the-application-frame-boundary-once
  (let* ((application-agent (make-test-agent))
         (application
         ...
         (let ((output
                 (openai:call-tool
                  ...
                  '(("count" . 12) ("mode" . "slow")
                    ("label" . "canvas"))
                  application-agent)))
           (true (= 1 (test-application-dispatch-count application)))
           (true (equal '(12 :slow "canvas") *test-command-result*))
           (true (search "CANVAS" output :test #'char-equal)))
      (agent:release-application-agent application-agent)
      (agent:wait-for-application-agent-release
       application-agent :timeout 1.0))))

(define-test release-is-terminal-idempotent-and-concurrent-safe
  (let ((close-count 0)
    ...
    (true (agent:wait-for-application-agent-release
           application-agent :timeout 1.0))
    (true (= 1 close-count))
    (true (eq :released (agent:application-agent-state application-agent)))
    (true (null (agent:application-agent-application application-agent)))
    (true (null (agent:release-application-agent application-agent)))
    (fail
...
(define-test close-failure-does-not-reattach-a-released-agent
  (let* ((close-count 0)
        (application-agent
          (make-test-agent
           :close-function
    ...
    (true (agent:wait-for-application-agent-release
           application-agent :timeout 1.0))
    (true (= 1 close-count))
    (true (typep (agent:application-agent-close-error application-agent)
                 'error))
    (true (eq :released
              (agent:application-agent-state application-agent)))
    (true (null (agent:application-agent-application application-agent)))

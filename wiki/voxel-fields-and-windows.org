...
* NEXT Pack the lighting worklist, then test level scheduling
:PROPERTIES:
:ID: QS1ERH
:END:
...
- first preserve the current LIFO order in a reusable array-backed worklist so
  representation cost is measured independently of scheduling;
- account for bytes per pushed and popped site and confirm that frontier
  growth, reuse, and emptying do not retain obsolete entries;
- then compare the same fixture with sixteen level buckets, consuming higher
  propagation levels first, to learn whether scheduling lowers the current
  7.32 visits per resident cell; and
- exercise from-scratch sky and block propagation plus incremental removal and
  reseeding, including cross-chunk and unavailable boundaries.

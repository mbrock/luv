...
The new [[file:../luvcraft/world.lisp][=vec3=]] structure is therefore deliberately modest.  It bundles
continuous CPU components and supplies component, scale, dot, cross, length,
and normalization operations.  It does not claim that a point is a velocity,
...
| =light-region= | eager or lazy solver atlas | world decomposition, chunk keys, offsets, faces, and lazy entry capture |

The duplication is visible in current source rather than inferred from equal
array lengths.  [[file:../luvcraft/light.lisp][=light-region-locate=]] performs three floor divisions, constructs a
chunk key, resolves an entry, and rebuilds the dense offset.
=map-entry-face-cells= separately reconstructs boundary planes.  In
[[file:../luvcraft/mesher.lisp][=luvcraft/mesher.lisp=]], block and light access through the resolved neighborhood
repeat the relative decomposition and offset arithmetic, while the immutable
snapshot repeats expanded-halo addressing for both fields.
...
=light-region-locate= and the entry-face adapter no longer contain their own

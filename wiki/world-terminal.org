...
crosshair inside their projected quadrilateral.  With a modal focus, =Tab=
leaves it.  Focus narrows the camera from 70 to 50 degrees with a short ease and
hides the world crosshair; leaving reverses both cues.
...
and focused-event methods according to their own semantics; other protocols
can separately govern rendering, simulation, camera attachment, or vehicle
motion.  Removing a focused overlay or stopping the session leaves focus
before releasing its resources.

There is deliberately no universal Escape binding in this layer.  Escape is
meaningful terminal input.  =Tab= is reserved as the provisional shared world
interaction verb, and a future named gameplay action can replace its raw key
binding while continuing to call [[lisp:toggle-luvcraft-session-focus]].
...
* NEXT Bridge Ghostty render-state rows into the block wall
:PROPERTIES:
:ID: NTQ5KY
:END:
...
* NEXT Attach a live PTY independently of the render boundary
:PROPERTIES:
:ID: 9OFAII
:END:
...
and child without blocking the world frame loop.  The remaining device-side
work is richer composed text input; focus and event routing remain the wall
gadget's side of the seam.

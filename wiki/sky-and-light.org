...
Phases 0--2 now have an implementation in the codebase; the later crystal,
HDR, bloom, and directional-shadow sections remain a plan.  Each phase below
...
- The expression language has floats, =vec2..4=, arithmetic, =dot=, =mix=,
  ...
  =normalize=.  It still has no comparison/control-flow vocabulary or general
  uniform packing.  Uniform blocks intentionally consist only of aligned vec4
  lanes.
...
- A public =*crystal-block*= or equivalently named block kind has its own atlas
  tile, propagated light emission, and surface emission.
- The placeable palette, number-key range, and window title derive from the
  palette rather than the current hardcoded seven-material assumption.
- Placing and removing the crystal on both sides of a chunk boundary converges
  to the reference light field and remeshes only the affected products.

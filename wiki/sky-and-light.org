...
This page plans a sky and lighting system for luvcraft: an animated sky and
primary light, caves and overhangs which become dark for the right reason,
...
This is a plan rather than an implemented specification.  Each phase below has
a visible result and an evidence gate, so it can be revised from measurements
without undoing the preceding phases.
...
- The expression language currently has floats, =vec2..4=, arithmetic,
  =dot=, =mix=, =swizzle=, and texture sampling.  It has no extended-instruction
  import, comparison/control-flow vocabulary, or general uniform packing.
  Uniform blocks intentionally consist only of aligned vec4 lanes.
- [[file:../spir-v.lisp][=spir-v.lisp=]] has neither =OpExtInstImport= nor =OpExtInst=, and the
  structured module in [[file:../shader.lisp][=shader.lisp=]] has no import section between
  capabilities and the memory model.  These are small but real backend
  additions; extended math is not merely a few surface aliases.
...
- The block vertex shader reads six vec4 camera lanes.  The host independently
  allocates a hard-coded 96-byte buffer.  Adding sky lanes must also remove
  that duplicated size knowledge or validate it at construction.
- Fog is computed per vertex as =1 - (z/far)^2= without a clamp and then
  interpolated to the fragment stage.  The first use of =clamp= is therefore a
  correctness improvement in an existing shader, not speculative vocabulary.
- The visible material hardcodes its sun vector, sky fill, sun colour, and sky
  colour.  The same sky literal also appears in the camera uniform and the
  render-pass clear value.
- The mesh vertex is three =:float32x3= attributes: position, UV plus AO, and
  normal.  The Vulkan backend currently accepts only =:float32x3= vertex
  attributes.  A new =:vec3= light/material attribute fits that honest current
  API; a proposed =:vec2= attribute does not work without first extending it.
- Meshing already copies a one-cell block halo to an immutable worker
  snapshot.  Corner AO samples only that neighborhood, and mesh results are
  rejected when their dependency stamp is stale.
...
Replace the three hardcoded sky literals with this one frame result.  The
clear value is merely a safe fallback.
...
=*crystal-block*= carries light emission 14, surface emission 1.0, and its own
...
from the palette length instead of hardcoding 1--7.  A block removal or
placement feeds the ordinary content-change path: direct sky, blocklight,
light publication, mesh invalidation, and GPU replacement all complete
without a special crystal edit command.
...
* Later: directional shadows, not baked animated sunlight
:PROPERTIES:
:ID: SKLATR
:END:
...
1. *Shader mathematics.*  Extended-instruction structure, typed math
   operators, clamped existing fog, uniform-size validation, =spirv-val=, and
   live replacement proof.
2. *Sky.*  Session clock/profile, shared frame parameters, fullscreen live sky
   pipeline, animated material/fog, and pinned deterministic captures.
3. *Skylight.*  Light-field objects and revisions, explicit boundary/source
   semantics, reference and incremental solvers, snapshot light halo, raw
   vertex light, and caves/canopies visible in a vertical test world.
...
6. *Directional shadows, if the moving scene asks for them.*  A separate GPU
   pass measured and judged in motion.

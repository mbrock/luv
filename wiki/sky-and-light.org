...
** NEXT Sample the shadow map in the block material
:PROPERTIES:
:ID: MFBOFD
:END:
...
- The bind-group layout can carry atlas texture/sampler, shadow depth
  texture/sampler, and the frame uniform without hard-coding a one-texture
  layout.
- The block vertex shader exports shadow coordinates and receiver depth, or an
  equivalent fragment-available light-space value, without mixing projection
  policy into the core shader DSL.
- The block fragment shader multiplies only the direct sun term by
  ...
  semantics.
- A pinned gazetteer or smoke scene visibly shows block geometry casting
  real-time shadows, and the new image is inspected before accepting any hash
  change.

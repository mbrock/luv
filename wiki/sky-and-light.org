...
(#CHUKWD).  Each phase below has a visible result and an evidence gate, so it
can be revised from measurements without undoing the preceding phases.
...
field into the atlas's fourth channel, and the block material reads its
gradient with the four tile-clamped taps it needed anyway.

Every block kind in this world is opaque, so that channel was carrying
nothing.  #SKOPEN's decision was that bloom must not commandeer *scene* alpha,
and it does not; this is atlas alpha, and the shader declares
=:surface-relief= rather than =:opacity= so the sampled meaning is stated
rather than assumed.  Transparent materials would need that channel back, and
would then need a separate relief attachment.
...
live image can re-sculpt grass tufts or stone cracks and rebuild the atlas
...
was not commandeered; the relief channel #CHUKWD uses is atlas alpha, which is
a different surface and is declared as what it holds.

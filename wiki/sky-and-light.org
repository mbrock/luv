...
the codebase; HDR and bloom remain a plan.  Each phase below has a visible
result and an evidence gate, so it can be revised from measurements without
undoing the preceding phases.
...
- The 1024x1024 map covers a 128-world-unit square, so one texel is 0.125
...
- =tests/block-world.lisp= proves that a 0.01-world-unit camera translation
  leaves the first two light-space rows unchanged while a 0.25 translation
  crosses the snap grid.  =tests/shader.lisp= proves nine depth samples and
  valid SPIR-V.
...
- directional sun shadows use a separate camera-centered, texel-snapped depth
  pass and are accepted with both analytic and consecutive-play evidence;

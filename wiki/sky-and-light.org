...
** NEXT Shader source abstractions for reusable shadow expressions
:PROPERTIES:
:ID: DXYHT4
:END:
...
- =define-shader-abstraction= or an equivalent protocol expands source forms
  before ordinary expression parsing, while preserving useful source forms and
  lowering provenance.
- A first =shadow-visibility= abstraction expands into =sample=, =swizzle=,
  =-=, =step=, and optional tap averaging, with no new SPIR-V lowering method
  beyond the core math operators.
- The abstraction is written in terms of supplied depth map, sampler,
  coordinates, receiver depth, texel size, and bias, so sun and torch lights can
  provide different projection data later.

Done when: tests show that redefining a shadow abstraction changes the parsed
shader graph, preserves last-known-good behavior on bad edits, and produces a
valid SPIR-V module without adding a high-level shadow operator.

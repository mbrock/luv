...
glyph.  That cost is intentionally still literal: each glyph occurrence owns
a texture pair and each frame slot owns matching bind groups, providing the
baseline the cache and atlas iterations must remove.

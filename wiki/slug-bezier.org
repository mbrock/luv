...
* NEXT Teach the shared language Slug's structured band fold
:PROPERTIES:
:ID: HMNMBU
:END:
...
with phi-carried index and state.  The remaining work in this mark is the
Slug-shaped data boundary: integer/index representations, exact curve loads,
conditional accumulation, and the sorted-coordinate early exit.  Those needs
now extend one shared fold rather than motivating a shader-only control
language.

Conditional accumulation is shared now too.  =if= is a first-class value node
...
from the same shape on both GPU targets.  Integer/index representations, exact
loads, and an early-exit form remain rather than being hidden inside this node.
...
* TODO Pack arbitrary quadratic contours into Slug bands
:PROPERTIES:
:ID: TTE5JG
:END:
...
This is deliberately the pre-serialization contract.  The current bounds and
band membership use the Lisp coordinates.  Before GPU upload, the curve points
must be converted to their actual RGBA16F representation and the bounds and
memberships recomputed from those quantized values; otherwise the CPU could
exclude a curve which the shader sees crossing a band boundary.  Dense curve
and band encoding, upload, and rendering from those resources remain open.
...
* TODO Render a zpb-ttf glyph through Slug
:PROPERTIES:
:ID: DTBI99
:END:

Intent: reuse McCLIM's Lisp-native TrueType outline boundary while replacing
its =cl-vectors=/=cl-aa= rasterization with luv's Slug packer and GPU draw.

Evidence needed: load a known bundled TrueType font through =zpb-ttf=, capture
its quadratic segments and metrics, pack a glyph containing an inner contour,
and render it through the same public shader specification on Metal and the
Vulkan smoke path.  Keep shaping separate: HarfBuzz supplies glyph indexes and
positions for runs, while this step proves one indexed glyph's outline.

The font boundary is now working before any rasterizer.  The =luv/slug= system
depends directly on =zpb-ttf= and converts its resolved
...
This completes outline acquisition, not the mark: the packed glyph still has
to be serialized after half quantization, uploaded, and rendered by the
data-driven shader on Metal and Vulkan.

Done when: changing the glyph id changes only uploaded outline/vertex data, not
shader source; the GPU image is visually inspected and compared with a small
CPU winding reference; and dynamic dilation is tested separately on geometry
whose undilated quad actually loses edge coverage.

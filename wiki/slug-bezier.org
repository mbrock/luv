...
has a six-vertex unit quad and one 72-byte instance record per drawable glyph:
world origin, right/up edge vectors, outline bounds and band counts, and atlas
bases.  The vertex stage expands these records; the render pass binds the two
buffers and one atlas group, then calls =draw= once with the glyph count as the
instance count.
...
| text vertex-buffer data     |          2,376 B   |           864 B |
...
pair paid the 4096-texel minimum row; vertex data falls by 63.6 percent.  The
...
close-up evidence in #9G0Z19 subsequently falsifies edge correctness; this mark
records the completed derivative and selection mechanism, not a claim that its
current bounds agree with preprocessing.
...
This is already falsifying evidence.  The horizontal cuts through =h= and =e=
and the vertical slivers in =h=, =r=, and =d= coincide with band transitions,
not font extrema or terrain edges.  The implementation currently sends the
quad's 0.035-em padded outline bounds to the fragment shader, but
[[lisp:pack-slug-outline]] partitions the unpadded packed-outline bounds.  The
shader consequently maps a fragment into a different spatial partition than
the serializer used to build its curve list.  The next narrow experiment is to
carry the packed min/max separately from padded quad geometry and require this
close-up to lose those transition-aligned cuts before widening the oracle.

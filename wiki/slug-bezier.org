...
The temporary scale path projects the run center each frame.  It combines the
world-units-per-em model scale, camera focal length, viewport height, and
current view distance, then updates the dense pixels-per-em lanes.  This makes
camera approach and retreat behave correctly enough for the world proof while
remaining an explicit precursor to fragment derivatives.
...
Frame metrics also count its current six vertices and one draw per drawable
glyph.  The cache iteration below removes repeated outline ownership, while
the atlas iteration must still remove this literal bind-and-draw baseline.
...
string, and retains normalized, serialized, uploaded Slug outlines by canonical
font path and HarfBuzz glyph ID.  A run now owns only placement, dense vertex
data, layout, and pipeline; stopping the session destroys frame bind groups,
then runs, then the cache, then the device.

Repeated occurrences share the same resource object and each drawable frame
creates only one bind group for that cached object.  The default
="hello, world"= proof has 11 drawable occurrences but only 8 cached outline
texture pairs.  A second identical run reuses both the =slug-shaped-text= and
all eight device resources.  The Metal regression constructs both runs and
checks those identities before exercising the ownership-order teardown.

...
(asdf:load-system "luvcraft/light-reference")
(luvcraft.light-reference:compare-voxel-light-solvers
...
The retired solver remains only as test/reference evidence.  Incremental
invalidation, removal, and residency reconciliation have since joined the
addition fixpoint as compiled frontier programs (#K3WRD3).  Misspelled or
retired solver names signal rather than silently changing algorithms.  This
keeps differential evidence available without preserving a second operational
truth.
...
- The retired =execute-frontier-program= selector cost only two dispatches per
  solve; deleting it closed runtime policy but could not explain several
  milliseconds.

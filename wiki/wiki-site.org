...
bracket links with =file:=, =https:=, =id:= or no scheme, a bare =file:=
link to an image in the wiki directory as an inline image (a paragraph of
only that link becomes a figure), the light =#ABC123= mention, and the six
emphasis markers with Org's pre- and post-character rules.  Anything else stays paragraph text, so an unfamiliar construct can
...
own =#+begin_src lisp= blocks, which can be read and drawn structurally with
the =<pre>= as fallback whenever reading fails.
...
* NEXT The site shows code references to figures
:PROPERTIES:
:ID: PF39II
:END:
...
Evidence: two such mentions exist today and are invisible to the site.  A
plain-text scan would already find them; the definition index and renderer
are what make the reference readable without leaving the page.
...
mention is reported like a dangling wiki mention.  A first cut may render the
definition as a =<pre>= if the dexp renderer is not ready; the mark stays
open until it is.

* TODO Read luv sources with Eclector into a definition index
:PROPERTIES:
:ID: YFDCSB
:END:
...
Evidence: none yet; Eclector and concrete-syntax-tree are in nixpkgs.
...
Evidence: none yet; wisp's =render-sexp= and =index.css= are the reference.
...
screenshot above can be reproduced from luv's own code.

...
The stylesheet knows only roles: =.bindings= is a CSS grid, its =.clause=
children subgrids spanning two columns, =.rest= a flowing cell, =.pair= an
inline unit, =.body= full width, =.break= a zero-height row break before a
=row-start=.  Adding a
form's layout is one =define-layout= line or one class with two methods,
and nothing else changes.

* NEXT Boxes hug their widest row
:PROPERTIES:
:ID: 993QQQ
:END:
...
/Done when./ =render-children-with-roles= wraps the leading non-body
children in a =.head= span and marks the list =stacked=; the stylesheet
draws =.stacked= as that grid; the dexp tests know the head wrapper; the
source pages show hugging boxes at every depth, and top-level definitions
still align at the left.

* NEXT Clauses as tables: cond, case, typecase, handler-case
:PROPERTIES:
:ID: 4175NC
:END:
...
/Done when./ =clauses-layout= has a =clause-head-count= (1 for =cond=,
=case=, =typecase=; 2 for =handler-case= and =restart-case=, so =error
(c)= is the key with its lambda list) and its stacked clauses render as
key cell and =.rest= cell; the stylesheet aligns them; tests cover a
=cond= and a =handler-case=; the source pages show the tables.
...
/Done./ [[file:../wiki-css.lisp][wiki-css.lisp]] is the compiler: a rule is =(selector . body)=, body
alternates =:property value= with nested rules whose selectors combine
with the parent's (=&= standing for it, groups crossed with groups) and
=(:media "query" …)= blocks hoisted around what they wrap; a value is a
string verbatim, a number, a symbol downcased with =--name= meaning
=var(--name)=, or a list, space-separated unless its head names a CSS
function, when it is a call with comma-separated arguments.  Units read
as symbols so they are written bare (=2.75rem=, =0.5fr=, =94%=); colours
are strings because =#f= is a reader macro.  =define-style= names a group
under a docstring and keeps its position on redefinition; =stylesheet-text=
compiles the groups in order; =write-stylesheet= is called by =write-site=
and by =render-op= on the system, whose inputs include the two style
files.  [[file:../wiki-style.lisp][wiki-style.lisp]] holds the whole sheet in twenty groups.  The
generated =style.css= was compared to the hand-written file rule by rule
after normalising whitespace and comments: 186 rules, identical
declarations, identical order.  The static file is gone; the tests cover
values, nesting, and media queries.
...
/Intent./ Smaller layout wins in the order they pay: =setf=, =setq=,
=psetf= as a two-column grid of place and value when there is more than
one pair; keyword pairs on rows aligned by a subgrid instead of a guessed
=min-width= for the key; =loop= as a keyword table, each clause a row
of keyword and rest as Emacs indents it; and function calls whose
arguments wrap continuing under the first argument.
...
for every =cl-source-file= that ASDF lists for the luv systems (pathnames
only, via =asdf:required-components=), each top-level form with its kind,
name, qualifiers or specializers, file, line range, comment text, and the
=#ID= mentions inside it.  Figures ask it who references them; =lisp:= links
ask it where a name is defined; the McCLIM browser can present it directly.
Reader conditionals and =#.= are rendered as visible tokens, never
evaluated.
...
render.  Each file of the =luv= and =luvcraft= systems becomes a page under
=source/=, drawn entirely as dexp boxes: a table of its definitions on top,
every top-level form anchored by its starting line (=#L956=), the file's
=in-package= hiding that package's prefix, comment blocks kept as one box
each.  [[source.html][source.html]] lists the files by system.  The reference graph runs

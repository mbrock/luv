...
diagrams in =#+begin_src mermaid= blocks (likewise), Typst-native diagrams in
=#+begin_src typst-diagram= blocks, bracket links with
...
* DONE Give printable pages and web diagrams one Typst source
:PROPERTIES:
:ID: WORWO1
:END:

/Intent./ Keep Org as the wiki's durable semantic source while adding a
high-quality paged output and a notation for diagrams which need not remain
ASCII art.

/Evidence./ =scripts/wiki pdf luft-world-model= renders the existing Org AST
through a small Typst backend and checked-in page template, producing a tagged,
eight-page PDF with headings as bookmarks and figure IDs as destinations.  The
same page contains two =typst-diagram= source blocks: compact CeTZ schematics
express the world-product flow and the chunk and primal lattices.  They remain
native Typst content in the PDF and become tightly bounded SVG resources in the
static or dynamic website.  Ordinary prose collapses Org's editorial source
wrapping rather than typesetting it as hard line breaks.  Lisp source blocks
use the same Eclector tree, operator layouts, and child roles as the web dexp:
Lisp emits semantic nodes, then a proportional Typst renderer measures only
token leaves, composes exact row, column, and frame bounds analytically, and
retains their width/height Pareto frontier.  There is deliberately no fill-style
mixed wrapping.  Every candidate has a top baseline, so an operator sits beside
its nested form's first row and the rectangle measures remain compositional.
Other example blocks remain plain monospace text.  Internal page names use
small capitals.  The page template
uses the private, annexed Equity family when its content is present and falls
back to Libertinus in ordinary public checkouts.  [[file:luft-star-figures.org][One star in four views]]
adds the first production-data plate: a =star-plate= CLOS value emits current
atlas geometry directly, and one CeTZ projection compares its occupancy, whole
geometry, polygon structure, and ownership on paper and web.
The follow-up [[file:luft-region-figures.org][From occupancy to one owned mesh]] is authored as an executable
=lisp-figure= block whose =region-plate= CLOS value traces three cells through
sixteen per-site owned patches into their 136-triangle composition.

The executable boundary is intentionally small.  A page declares one
=#+lisp-package=; each trusted =lisp-figure= block contains exactly one form
and must return a =visual-figure=.  =write-figure-typst= emits one deterministic
fragment which the PDF embeds and the web compiler turns into SVG.  Ordinary
=lisp= blocks remain inert source.  Resource discovery does not evaluate
figures; site preparation evaluates each occurrence once and reports page and
block context on failure.  Figure classes register non-Lisp renderer inputs so
editing a =.typ= module invalidates the static site correctly.

/Done when./ One substantive page builds as PDF, both kinds of diagram render
legibly in that PDF and the real web reading view, and the ordinary wiki tests
cover source generation and diagram resource discovery.
...
* DONE Give web pages semantic view objects
:PROPERTIES:
:ID: CZZJDQ
:END:

Intent: make the site shell and its broad layout policies inspectable Lisp
objects rather than contracts reconstructed from callback nesting and body
selectors.  Keep geometry in CSS: a view chooses reading, workspace, or
sidebar semantics, while shared frames and a small token vocabulary establish
alignment and rhythm.

Evidence: =page-view= and its three layout subclasses own title, site,
breadcrumbs, tools, context, and content.  =render-view= fixes the shell's
header/status/main/footer ownership; generic content and tool methods let
document, Source, and Activity views specialize it.  The old frame function is
now a compatibility adapter for Showcase, Stars, and Bodies.  Shared
=page-frame= regions and spacing, measure, gutter, rail, and workspace tokens
give the resulting roles one geometric source of truth.

Done when: reading and workspace views dispatch through classes, the three
shell regions align at desktop width, contributed pages still render through
the adapter, and tests assert both view policy and whole-page ownership.

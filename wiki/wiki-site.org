...
Lisp emits semantic nodes, then a proportional Typst renderer measures complete
row and column alternatives and retains their width/height Pareto frontier.
There is deliberately no fill-style mixed wrapping.  Every candidate has a
top baseline, so an operator sits beside its nested form's first row and the
rectangle measures remain compositional.  Other example blocks remain plain
monospace text.  Internal page names use small capitals.  The page template

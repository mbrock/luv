...
ten-page PDF with headings as bookmarks and figure IDs as destinations.  The
same page contains two =typst-diagram= source blocks: Fletcher expresses the
world-product flow and CeTZ draws the chunk and primal lattices.  They remain
native Typst content in the PDF and become tightly bounded SVG resources in
the static or dynamic website.  The remaining example blocks still render as
plain monospace text; migration is deliberate rather than automatic.

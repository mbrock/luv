...
Execution placement is deliberately outside this graph.  A GPU worker may run
the same command through SSH, CI, or a future queue; its hostname and checkout
path are deployment configuration, never recipe data.  Likewise Git annex is
one possible publication cache, not part of capture identity: an annex pointer
alone cannot serve a browser, so deployment would still have to fetch and copy
the content.  Object storage, release assets, annex, and retention policy can
be compared after the local build and transclusion contract has produced real
artifacts.
...
* IDEA Publish generated media out of band
:PROPERTIES:
:ID: 0BGP3P
:END:
...
source-revision manifest.  What remains is operational evidence comparing an
annex-backed deployment fetch, object storage, and release artifacts, including
cache invalidation, retention, and Pages URLs.

/Done when./ A clean non-GPU site build can obtain the manifest and referenced
media, a configured remote executor can publish them without repository host
names, and pruning policy cannot break still-linked historical figures.

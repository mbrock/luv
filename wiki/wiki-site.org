...
Intent: the first rung of #F6B3NI.  Scan the Lisp sources ASDF knows about
for =#ID= mentions during =render-op= on the system and render a "Referenced
from" line per figure with a GitHub line link, so #PLRP3A and #LNRY72 pay off.

Evidence: two such mentions exist today and are invisible to the site.

Done when: those two figures show their source references on the site, the
scan is covered by a test, and a dangling code mention is reported like a
dangling wiki mention.

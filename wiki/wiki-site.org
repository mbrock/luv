...
a flex-wrapping =div.list= with side borders; roles come from an operator
table (=*operator-head-counts*=) plus =defmethod='s qualifier scan and a
=*binding-operators*= list, and are emitted as =operator=, =body=,
=bindings=, and =clause= classes, comments not counting as arguments.  The

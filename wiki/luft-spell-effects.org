...
The first fireball was a playable experiment, now retired; this page preserves
its visual study and proposals for future spell effects, not an active spell
system. Torch flames and the wizard, staff, and orb appearance remain. A moving
analytic ball was not yet the visual proposition. The reference below suggests something more
...
1. Add a future analytic core to the motion-bearing scene pass alongside the
   player, with exact current and previous endpoints and motion output. There
   is no longer a fireball shader path to reuse. Temporal reconstruction can
   then stabilize the core's subpixel edge.

...
* TODO Sweep the 256 vertex stars with the integer oracle
:PROPERTIES:
:ID: WJRRK7
:END:

/Intent./ Build the vertex-star table that #8UGGY0's fan stream reads:
for each of the 256 stars, derive the incident bands, their offset-line
intersections, the turn count $k$, and the tile fan, entirely in integer
arithmetic; state the width and planarity predicates explicitly rather than
authoring entries by eye.
...
Arc policy (#GT4Y9J), which suggests the alphabet is closed — that claim is
exactly what the sweep must decide.

/Done when./ The table exists as data with its generating predicate in the
repository; every entry is checked against Blender's Arc bevel of the
corresponding voxel star by exact vertex-set comparison; any star that
falls outside the tile alphabet is documented as its own figure rather

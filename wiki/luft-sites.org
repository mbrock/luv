...
the domain on which Blender is a specification.  The repository carried a
headless generator, =scripts/luft-blender-star-oracle.py=, and its canonical
mapping, =luft/blender-arc-stars.sexp= (both retired by #Z0X3YQ).  It
constructed the welded cubical boundary for every one of the 256 vertex
stars, applied the recorded one-segment Arc modifier, extracted the central
junction, and checked integer coordinates, the 26-normal alphabet, all 24
proper rotations, and occupancy complement.  The corpus was regenerated and
compared with =make luft-blender-oracle-check=.
...
* DONE The Arc oracle is retired; the derivation replaced it
:PROPERTIES:
:ID: Z0X3YQ
:END:

The corpus of #TQ8FUN did its job and is gone.  Once the width-one geometry
was understood well enough to derive from first principles — inset faces,
transition-paired bands and collars, and the junction dichotomy of #SAMH68 —
the executable oracle became =luft/star-geometry.lisp= itself, a readable
in-repo derivation checked against the compiled production tables for all
256 stars by the exhaustive differential in LUFT's tests.  The generator
script, the =blender-arc-stars.sexp= mapping, and the =make= targets are
deleted.  The one fact production code still drew from the corpus — which
sheet masks are /regular/ — proved to be exactly the complement of
[[lisp:luft::star-singular-p]] over all 256 masks, so =decompose-star-mask=
now asks that predicate directly.  What Blender contributed survives as the
census language of this page and the regular/singular split it certified.
...
* Material contacts render as per-patch flips, not trim
:PROPERTIES:
:ID: DH3HR8
:END:

At bevel 1/8 in the sanctuary, flush grass/stone ground reads as tiles
with square, half-square, and quarter-square bites of the wrong material
at cell-regular intervals, and a crystal set on a stone post carries one
stone sliver in its base ring.  These are measured behaviors of the
current width-one path, recorded here for the site-stream rewrite.
Facts, each from a small fixture meshed through its scene's material
program:

- Flush seam: a lawn beside a level architecture strip puts grass
  junction squares (one tick) on the stone tops at every cell corner
  along the seam, and limestone bands (one tick), limestone corner
  squares, and whole limestone face sub-quads on the grass tops.  Both
  directions of contamination occur at once on a straight, flat seam.
- Horizontal material seam (crystal on stone post): flat sides split at
  the seam and keep their own cell's material; the four vertical corner
  junctions span both cells, must choose one material, and resolve to the
  dressed-limestone contact.  Identical patch inventory at widths one,
  two, and four; at medial four the same patches are the whole collar and
  read as intentional (=screenshots/crystal-bevels/= holds the sweep).
- A wall standing on a lawn: straight concave bases emit no mixed
  geometry; each exterior wall corner emits one junction pinwheel of
  foundation contact on the flat grass.

The mechanism, strictly as measured: near a seam a flat surface is
emitted as face pieces, one-tick rim bands, and one-tick corner
junctions; each patch resolves its material independently through its
own vertex-star closure, so any patch whose star touches the other
material flips entirely.  No trim is drawn anywhere.  The direction of a
flip comes from the pairwise host reduction
([[lisp:luft.render:material-closure-surface-assembly]], stone-primary
for earth-plus-stone); redefining that one method to earth-primary moves
the bites from the grass side to the stone side, observed live in one
renderer refresh:

[[file:images/earth-stone-stone-primary.png]]

[[file:images/earth-stone-earth-primary.png]]

To observe: =./sly play luft=, bevel 1/8, walk the courtyard where lawn
meets the podium and tower shoes.  To reproduce in data, mesh a fixture
through its own material program and dump triangle records:

#+begin_src lisp
(let ((b (luft.render::make-scene-builder :horizontal-bits 5)))
  (luft.render::scene-builder-box b 4 14 4 14 0 4)
  (luft.render::scene-builder-box b 9 11 4 14 0 4 :architecture-p t)
  (let* ((scene (luft.render::finish-scene-builder b))
         (mesh (luft.render::%make-scene-union-mesh
                scene (luft.render::scene-solid scene) 1 nil
                (luft.render::make-compiled-material-chamfer-stock-function
                 (luft.render::scene-material-program scene)))))
    (luft::%map-surface-mesh-triangle-records
     (lambda (kind stock ambient mask normal a b c)
       (declare (ignore ambient mask normal))
       ;; Filter by plane and region; resolve STOCK through
       ;; SURFACE-ASSEMBLY-AT to name each patch's material.
       (list kind stock a b c))
     mesh)))
#+end_src

** TODO Give flat faces their own material and contacts their own geometry
:PROPERTIES:
:ID: 4UY1IC
:END:

/Intent./ In the site-stream mesher, flat face pieces take their material
unconditionally from their own cell; contact material may appear only on
geometry that belongs to the contact, with a declared owner per material
pair, eventually as a continuous trim of chosen width in the spirit of
#41HEJ1.  Verify on the sanctuary courtyard that seam tiles are solid and
the crystal base ring is unbroken.
...
precise baseline meaning of “SIMD contributes about 2%”: SIMD owned its small
operator, but the query gave that operator too little total work to move
end-to-end latency.  #IS4S0L and #LQSV44 subsequently removed the repeated
halo, Z-bound, occupancy, and authored-material reconstruction which this
profile exposed.  Warm builds now take about 10.4 ms and 2.1 MiB per chunk;
planning (5.8 ms) and projection (4.3 ms) are the remaining measured frontier.
The same query path now serves uniform bevel widths one through three; medial
width four retains the general reference path.

* DONE Compile query-native topology dimension rows
:PROPERTIES:
:ID: 3IKS3Y
:END:

The ordered prerequisites in #PWUMY7 are complete: chain-local occupancy facts
and chain-order material lanes survive warm remeshes, and the cohort benchmark
proves their one-edit invalidation boundaries.  Planning and projection now
account for almost all of the 10.4 ms warm build, so topology rows are the next
measured representation experiment rather than a speculative cleanup.
...
/Evidence./ One immutable CSR-style dimension now holds every mask's face,
band, and fan templates, compact material references, source witnesses, and
ambient flags.  The planner appends these rows directly for interior sites and
performs the half-open source-ownership test only on seam sites.  A complete
compiler test reconstructs all rows from the retained descriptors, while the
existing 256-star, width-one-through-three, and real-chunk seam differentials
retain the exact reference census.  On the same mountain-sanctuary fixture,
planning fell from 5.120 to 0.620 ms/chunk and the warm aggregate from 11.556
to 6.445 ms/chunk with allocation unchanged at 2.110 MiB/chunk.

* DONE Implement the site-stream LUFT renderer backbone
:PROPERTIES:
:ID: RO2OCL
:END:
...
/Outcome./ The production mesher and renderer now retain three typed instance
streams: inset face quads, crease-band quads, and junction-fan triangles.  One
immutable fixed-point template vocabulary supplies their integer offsets;
instances carry only their lattice anchor, template id, and compact material
state.  [[lisp:luft.render::make-render-population][The render population]] preserves every source instance exactly while
combining all resident meshes into at most one triangle draw and one quad draw.
The playable mountain sanctuary streams these chunk products through the
ordinary Metal and Vulkan atelier, rather than through a fixture-only path.

Exact tests close every emitted mesh by integer edge equality, cover the
trapezoid turn at the wall-end family, and keep the =06=, =18=, and =69=
singular fixtures closed and nondegenerate.  Uniform widths one through three
share the finite columnar query and differ only by their immutable template
vocabulary; medial width four remains a separately checked realization.  The
old face-patch and intermediate integer-triangle ABIs are both historical.
Further mesher work belongs to the query execution marks #3IKS3Y and #GPL9Z3,
not to rebuilding this renderer backbone.

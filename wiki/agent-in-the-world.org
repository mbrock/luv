...
* NEXT Wire three commands into an agent and draw their cassettes
:PROPERTIES:
:ID: YDAQNO
:END:
...
schema, tool bodies =present= to a recording stream read under
=+model-view+= (#WS5OEX), and a =cassette-view= draws each call on the HUD
while it runs and commits it when done (#9K823O, #14ZP6S).  Three commands
are enough to see the whole shape: =eval= against the image, =look= at a
handle, and =place-block=.  Parameterized M-x should light up from the same
parser-table walk.

/Evidence./ [[file:../openai/agent.lisp][openai/agent.lisp]] already runs a
turn with =call-tool= dispatch on a =tool= class, so the provider side needs
no change to start.  The command tables and M-x walk exist in
=luvcraft/clim/=.  The HUD overlay path exists for the legend and tape
prompt.  The rest of this page is the design.

/Done when./ An agent asked to put a stone block next to the player does
it through =com-place-block= via =accept-from-string=, its three calls
appear as cassettes on the HUD and then on a wall, and a =./sly= session
with no window gets the same transcript as plain lines.  The evidence
paragraph then records what the model actually did with the handles.

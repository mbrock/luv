...
The body (#5JDNKF) is now deliberately skew to the block field.  A =gnome=
owns an integer =(x y z)= cell and belongs to =*agents*=, not =*critters*=;
the cell is its authoritative place while its visible animation is derived
presentation state.  =Spawn Agent= chooses an empty supported cell ahead of
the player without editing terrain.  The old =gnome-block-kind= and its atlas
tiles no longer exist.

The first visible body is a retained McCLIM drawing replayed as a
camera-facing world widget: a small red-hatted figure with a gentle visual
bob.  This gives agents a scene render path independent of terrain without
prematurely choosing a geometric or SDF body language.  Its upright world
volume answers the view ray directly, and a nearer terrain hit occludes it.
Focusing one with TAB steps the camera back to a conversational distance,
aims it at the face with the focused field of view, and turns the keys into a
...
* NEXT Parameterized M-x and agents that take discrete steps
:PROPERTIES:
:ID: YPGXKI
:END:
...
definition reachable by key, M-x, and tool.  Second, give an embodied agent a
=step= command whose state transition is between integer cells.  The body and
bubbles may interpolate between old and new cells, but continuous velocity is
not part of the agent's authoritative state.

/Evidence./ #V9DHV5 records the 2026-08-19 unblocking: =*agents*=,
=spawn-agent=, direct body targeting, and the independent world-widget body.
The M-x gap is

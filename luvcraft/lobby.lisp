...
(defparameter +lobby-api-key-topic+ "luv/store/OPENAI_API_KEY")
     ...
     (let ((id (subseq topic (length +lobby-presence-prefix+))))
       (unless (string= id (lobby-client-id client))
         (sb-thread:with-mutex ((lobby-client-lock client))
           ...
           (incf (lobby-client-revision client))))))
    ((string= topic +lobby-api-key-topic+)
     (sb-thread:with-mutex ((lobby-client-lock client))
       (if (zerop (length payload))
           (remhash "OPENAI_API_KEY" (lobby-client-values client))
           (setf (gethash "OPENAI_API_KEY" (lobby-client-values client)) payload))
       (incf (lobby-client-revision client)))))
  client)
    ...
    (unwind-protect
         (progn
           ...
           (mqtt.net:subscribe connection (list +lobby-presence-filter+ :qos 1)
                               (list +lobby-api-key-topic+ :qos 1))

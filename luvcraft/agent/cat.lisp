...
(defmethod embodied-agent-head-position ((cat cat) &optional (lift 0.0))
  (luvcraft::make-vec3 (luvcraft::body-x cat)
                       (+ (luvcraft::body-y cat)
...
(defmethod embodied-agent-face-position ((cat cat) &optional (lift 0.0))
  (luvcraft::make-vec3 (luvcraft::body-x cat)
                       (+ (luvcraft::body-y cat)
                          (* *cat-face-height* (cat-stature)) lift)
                       (luvcraft::body-z cat)))
               ...
               (if (zerop direction)
                   ...
                   (let ((entering (/ (- minimum origin) direction))
                     ...
                     (setf near (max near entering)
                           far (min far leaving))
                     (<= near far))))))
      (let ((center-x (luvcraft::body-x cat))
            (center-z (luvcraft::body-z cat)))
...
(defmethod embodied-agent-body-sphere ((cat cat))
  ...
  (let* ((phase (/ (get-internal-real-time)
         ...
         (stature (cat-stature)))
    (values (luvcraft::body-x cat)
            (+ (luvcraft::body-y cat) (* *cat-body-centre* stature) breath)
            (luvcraft::body-z cat)

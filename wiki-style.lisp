...
;;;; whose selectors are the roles wiki-dexp.lisp assigns.
...
(define-style library
  ...
  (".library h1"
   :margin 0
   :font (700 1.25rem/1.1 --display-font)
  ...
  (".door"
   ...
   :background --paper
   ("&:hover, &:focus-visible"
    :border-color (color-mix (in srgb) (--accent 70%) --rule)
    :text-decoration none
    :outline none)
   ("&.selected"
    :border-color --accent
  ...
  (".door-title"
   :font (700 0.95rem/1.2 --display-font))
  (".door-meta"
   :color --muted
   :font (500 0.82rem/1.2 --display-font)
   :min-width 0
...
readable measure."
  ...
  (".site-footer"
   ...
   :font (500 0.82rem/1.4 --display-font)
   :color --muted))
...
(define-style type
  ...
  (".mark-next" :background --mark-next)
  (".mark-wait" :background --mark-wait)
  (".mark-done" :background --mark-done)
  (".mark-idea" :background --mark-idea)
  ("p.backlinks"
   :font-size 0.85rem
   :color --muted
   :margin-top 1rem))
...
(define-style text-blocks
  ...
  ("code"
   :background none
   :padding 0
   :color (color-mix (in srgb) (--ink 80%) --accent))
  ("pre"
   :background --code-bg
   :padding (0.8rem 1rem)
   :overflow-x auto
...
borders are its parentheses; body forms take the full width and stack."
  (".lisp"
   ...
   (".keyword"
    :color --mark-idea)
   (".string"
    :color (color-mix (in srgb) (--ink 60%) --mark-done)
...
(define-style definitions-index
  ...
  ("li.definition-entry .kind-defclass, li.definition-entry .kind-defstruct, li.definition-entry .kind-define-condition"
   :color --mark-idea)
  ("li.definition-entry .kind-defgeneric" :color --accent)
  ("li.definition-entry .kind-defmacro" :color --mark-wait)
...
(define-style source-index
  ...
  ("table.systems"
   ...
   :line-height 1.35
   ("th"
    :font (700 0.72rem/1.4 --display-font)
    :text-transform uppercase
    :letter-spacing 0.06em
    :color --muted
  ...
  ("td.system-description"
   :max-width 28rem
   :color (color-mix (in srgb) (--ink 80%) --muted))
  ("td.system-depends"
   :max-width 16rem
   :color --muted)
  ("td.system-files"
...
(define-style code-prose
  ...
  (".lisp .prose"
   ...
   ("code"
    ...
    :font-size 0.85em))
  (".lisp .string.prose"
   :color (color-mix (in srgb) (--ink 75%) --mark-done)
   :padding-left 0.6em
   :border-left (2px solid (color-mix (in srgb) (--mark-done 40%) transparent)))
  (".lisp .comment.prose"
   :color --muted
   :font-style italic
   :padding-left 0.6em
...
(define-style figure-cards
  ...
  (".figure-card .card-title, .figure-popover .card-title"
   ...
   :font (700 1rem/1.25 --display-font)
   :letter-spacing -0.02em
   :color --ink)
  ...
  (".figure-popover .card-meta"
   ...
   :color --muted
   :font (500 0.8rem/1.3 --display-font)
   :border-bottom (1px solid --rule))
  ...
  (".figure-popover .card-excerpt"
   ...
   :text-align start
   :hyphens auto
   :color (color-mix (in srgb) (--ink 88%) --paper)))
...
(define-style systems-table
  ...
  ("button.count"
   :font (600 0.72rem/1.4 --display-font)
   :color --muted
   :background none
   :border (1px solid --rule)
   :border-radius 999px

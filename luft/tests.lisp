...
(defun %test-chunk-fibers ()
  (let* ((domain (make-world-domain :horizontal-bits 8))
         (builder (make-chain-builder domain))
         (random-state (sb-ext:seed-random-state 7)))
    ;; A rough terrain chunk at grid (1, 1) with an overhang and a shaft.
    (dotimes (i 64)
      (dotimes (j 64)
        (let ((x (+ 64 i)) (y (+ 64 j)))
          (dotimes (z (+ 30 (mod (* i j) 17)))
            (unless (and (= i 10) (= j 10) (< 5 z 20))
              (chain-builder-add-site
               builder (make-site domain x y z +cell-extent+ 1))))
          (when (= i 20)
            (chain-builder-add-site
             builder (make-site domain x y 100 +cell-extent+ 1))))))
    (let* ((chain (finish-chain-builder builder))
           (fibers (chain-fibers chain)))
      (%check (eq fibers (chain-fibers chain)))
      (%check (= (chain-count chain) (fibers-cell-count fibers)))
      (%check (chain= chain (fibers-chain fibers)))
      (dotimes (i 2000)
        (let ((x (+ 64 (random 64 random-state)))
              (y (+ 64 (random 64 random-state)))
              (z (random 120 random-state)))
          (%check (= (chain-cell-occupancy-bit chain x y z)
                     (fibers-cell-bit fibers x y z)))))
      ;; The shaft at (74, 74): air for 6..19, solid below and above.
      (%check (fibers-column-clear-p fibers 74 74 6 20))
      (%check (not (fibers-column-clear-p fibers 74 74 5 20)))
      (%check (not (fibers-column-clear-p fibers 74 74 6 21)))
      (%check (= 5 (fibers-highest-cell-below fibers 74 74 20)))
      (%check (= 20 (fibers-lowest-cell-above fibers 74 74 6)))
      (%check (= 100 (fibers-lowest-cell-above fibers 84 70 90)))
      (%check (null (fibers-lowest-cell-above fibers 84 70 101)))
      (%check (null (fibers-highest-cell-below fibers 84 70 0)))
      (%check (fibers-column-clear-p fibers 84 70 101 400))
      (%check (fibers-column-clear-p fibers 84 70 -5 0))
      ;; Edits are fresh values; the original is untouched.
      (let ((edited (fibers-with-cell fibers 74 74 10 1)))
        (%check (= 1 (fibers-cell-bit edited 74 74 10)))
        (%check (= 0 (fibers-cell-bit fibers 74 74 10)))
        (%check (= (1+ (chain-count chain)) (fibers-cell-count edited)))
        (%check (chain= (fibers-chain edited)
                        (chain+ chain
                                (finish-chain-builder
                                 (let ((b (make-chain-builder domain)))
                                   (chain-builder-add-site
                                    b (make-site domain 74 74 10
                                                 +cell-extent+ 1))
                                   b))))))
      ;; Fibers mesh identically to their chain.
      (handler-bind ((missing-chunk
                       (lambda (c) (declare (ignore c))
                         (invoke-restart 'treat-as-air))))
        (let ((from-chain (mesh-star-chunk chain (chunk-key-at 64 64)
                                           :outside-domain-policy :air))
              (from-fibers (mesh-star-chunk fibers (chunk-key-at 64 64)
                                            :outside-domain-policy :air)))
          (%check (equalp (surface-mesh-star-site-words from-chain)
                          (surface-mesh-star-site-words from-fibers)))))
      ;; A store answers world probes and edits per chunk.
      (let ((store (make-fiber-store-from-chain chain)))
        (%check (= 1 (fiber-store-count store)))
        (%check (= 1 (fiber-store-cell-bit store 70 70 3)))
        (%check (= 0 (fiber-store-cell-bit store 74 74 10)))
        (%check (chain= chain (fiber-store-chain store)))
        (%check (eq :missing
                    (handler-case (fiber-store-cell-bit store 10 10 3)
                      (missing-chunk () :missing))))
        (%check (= 1 (handler-bind ((missing-chunk
                                      (lambda (c) (declare (ignore c))
                                        (invoke-restart 'treat-as-solid))))
                       (fiber-store-cell-bit store 10 10 3))))
        (%check (eq :outside
                    (handler-case (fiber-store-cell-bit store -1 10 3)
                      (outside-domain () :outside))))
        (fiber-store-edit-cell store 74 74 10 1)
        (%check (= 1 (fiber-store-cell-bit store 74 74 10)))
        (fiber-store-edit-cell store 10 10 3 1)
        (%check (= 2 (fiber-store-count store)))
        (%check (= 1 (fiber-store-cell-bit store 10 10 3)))
        (%check (= 3 (fiber-store-lowest-cell-above store 10 10 0)))
        (%check (= 3 (fiber-store-highest-cell-below store 10 10 50)))
        (%check (fiber-store-column-clear-p store 10 10 4 255))))))
